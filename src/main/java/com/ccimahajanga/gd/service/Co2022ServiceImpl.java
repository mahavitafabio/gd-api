...
@Service
public class Co2022ServiceImpl implements Co2022Service {

	@Autowired
	private Co2022Repository co2022Repository;
	
	@Override
	public Iterable<Co2022> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return co2022Repository.findByProduitContaining(q);
		}
		return co2022Repository.findAll();
	}

	@Override
	public void save(Co2022 co2022) {
		// TODO Auto-generated method stub
		co2022Repository.save(co2022);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			co2022Repository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Co2022> co2022List = co2022Repository.findAll();
		int i = 1;
		for (Co2022 co2022 : co2022List) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(co2022.getCo2022Id());
			cell = row.createCell(1);
			cell.setCellValue(co2022.getNumero());
			cell = row.createCell(2);
			cell.setCellValue(co2022.getDate());
			cell = row.createCell(3);
			cell.setCellValue(co2022.getNomEntreprise());
			cell = row.createCell(4);
			cell.setCellValue(co2022.getAdresse());
			cell = row.createCell(5);
			cell.setCellValue(co2022.getDestination());
			cell = row.createCell(6);
			cell.setCellValue(co2022.getProduit());
			cell = row.createCell(7);
			cell.setCellValue(co2022.getQuantiteExporte());
			cell = row.createCell(8);
			cell.setCellValue(co2022.getUnite());
			cell = row.createCell(9);
			cell.setCellValue(co2022.getNombreConteneur());
			cell = row.createCell(10);
			cell.setCellValue(co2022.getPrixUnitaire());
			cell = row.createCell(11);
			cell.setCellValue(co2022.getMontant());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
        ...
        while (rowIterator.hasNext()) {
            ...
            Co2022 cerfit = new Co2022();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                cerfit.setCo2022Id(Integer.parseInt(cellValue));
                cell = cellIterator.next();
            }
            
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            cerfit.setNumero(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setDate(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setNomEntreprise(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setAdresse(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setDestination(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setProduit(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerfit.setQuantiteExporte(Integer.parseInt(cellValue));
            }

            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setUnite(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerfit.setNombreConteneur(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerfit.setPrixUnitaire(cellValue);
