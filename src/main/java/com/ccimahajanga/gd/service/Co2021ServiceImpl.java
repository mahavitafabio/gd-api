...
@Service
public class Co2021ServiceImpl implements Co2021Service {

	@Autowired
	private Co2021Repository co2021Repository;
	
	@Override
	public Iterable<Co2021> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return co2021Repository.findByProduitContaining(q);
		}
		return co2021Repository.findAll();
	}

	@Override
	public void save(Co2021 co2021) {
		// TODO Auto-generated method stub
		co2021Repository.save(co2021);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			co2021Repository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Co2021> co2021List = co2021Repository.findAll();
		int i = 1;
		for (Co2021 co2021 : co2021List) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(co2021.getCo2021Id());
			cell = row.createCell(1);
			cell.setCellValue(co2021.getNumero());
			cell = row.createCell(2);
			cell.setCellValue(co2021.getDate());
			cell = row.createCell(3);
			cell.setCellValue(co2021.getNomEntreprise());
			cell = row.createCell(4);
			cell.setCellValue(co2021.getAdresse());
			cell = row.createCell(5);
			cell.setCellValue(co2021.getDestination());
			cell = row.createCell(6);
			cell.setCellValue(co2021.getProduit());
			cell = row.createCell(7);
			cell.setCellValue(co2021.getQuantiteExporte());
			cell = row.createCell(8);
			cell.setCellValue(co2021.getUnite());
			cell = row.createCell(9);
			cell.setCellValue(co2021.getNombreConteneur());
			cell = row.createCell(10);
			cell.setCellValue(co2021.getPrixUnitaire());
			cell = row.createCell(11);
			cell.setCellValue(co2021.getMontant());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
        ...
        while (rowIterator.hasNext()) {
            ...
            Co2021 cerf = new Co2021();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                cerf.setCo2021Id(Integer.parseInt(cellValue));
                cell = cellIterator.next();
            }
            
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            cerf.setNumero(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setDate(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setNomEntreprise(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setAdresse(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setDestination(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setProduit(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerf.setQuantiteExporte(Integer.parseInt(cellValue));
            }

            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setUnite(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerf.setNombreConteneur(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerf.setPrixUnitaire(cellValue);
