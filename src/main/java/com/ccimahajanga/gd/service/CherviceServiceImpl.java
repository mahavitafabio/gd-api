...
@Service
public class CherviceServiceImpl implements CherviceService {
	
	@Autowired
	private CherviceRepository cherviceRepository;
	
	@Override
	public Iterable<Chervice> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return cherviceRepository.findByNomEntrepriseContaining(q);
		}
		return cherviceRepository.findAll();
	}

	@Override
	public void save(Chervice chervice) {
		cherviceRepository.save(chervice);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			cherviceRepository.deleteById(id);
		}
	}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Chervice> cherviceList = cherviceRepository.findAll();
		int i = 1;
		for (Chervice chervice : cherviceList) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(chervice.getServicesId());
			cell = row.createCell(1);
			cell.setCellValue(chervice.getNomEntreprise());
			cell = row.createCell(2);
			cell.setCellValue(chervice.getAdresseEntreprise());
			cell = row.createCell(3);
			cell.setCellValue(chervice.getActivitePrincipale());
			cell = row.createCell(4);
			cell.setCellValue(chervice.getContact());
			cell = row.createCell(5);
			cell.setCellValue(chervice.getNomResponsable());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
		List<String> headers 
		= new ArrayList<>(Arrays.asList("SERVICE_ID",
        ...
        while (rowIterator.hasNext()) {
            ...
            Chervice ent = new Chervice();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                ent.setServicesId(Integer.parseInt(cellValue));
