...
@Service
public class Co2023ServiceImpl implements Co2023Service {

	@Autowired
	private Co2023Repository co2023Repository;
	
	@Override
	public Iterable<Co2023> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return co2023Repository.findByProduitContaining(q);
		}
		return co2023Repository.findAll();
	}

	@Override
	public void save(Co2023 co2023) {
		// TODO Auto-generated method stub
		co2023Repository.save(co2023);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			co2023Repository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Co2023> co2023List = co2023Repository.findAll();
		int i = 1;
		for (Co2023 co2023 : co2023List) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(co2023.getCo2023Id());
			cell = row.createCell(1);
			cell.setCellValue(co2023.getNumero());
			cell = row.createCell(2);
			cell.setCellValue(co2023.getDate());
			cell = row.createCell(3);
			cell.setCellValue(co2023.getNomEntreprise());
			cell = row.createCell(4);
			cell.setCellValue(co2023.getAdresse());
			cell = row.createCell(5);
			cell.setCellValue(co2023.getDestination());
			cell = row.createCell(6);
			cell.setCellValue(co2023.getProduit());
			cell = row.createCell(7);
			cell.setCellValue(co2023.getQuantiteExporte());
			cell = row.createCell(8);
			cell.setCellValue(co2023.getUnite());
			cell = row.createCell(9);
			cell.setCellValue(co2023.getNombreConteneur());
			cell = row.createCell(10);
			cell.setCellValue(co2023.getPrixUnitaire());
			cell = row.createCell(11);
			cell.setCellValue(co2023.getMontant());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
        ...
        while (rowIterator.hasNext()) {
            ...
            Co2023 cerc = new Co2023();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                cerc.setCo2023Id(Integer.parseInt(cellValue));
                cell = cellIterator.next();
            }
            
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            cerc.setNumero(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setDate(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setNomEntreprise(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setAdresse(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setDestination(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setProduit(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerc.setQuantiteExporte(Integer.parseInt(cellValue));
            }

            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setUnite(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	cerc.setNombreConteneur(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            cerc.setPrixUnitaire(cellValue);
