...
@Service
public class Co2020ServiceImpl implements Co2020Service {

	@Autowired
	private Co2020Repository co2020Repository;
	
	@Override
	public Iterable<Co2020> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return co2020Repository.findByProduitContaining(q);
		}
		return co2020Repository.findAll();
	}

	@Override
	public void save(Co2020 co2020) {
		// TODO Auto-generated method stub
		co2020Repository.save(co2020);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			co2020Repository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Co2020> co2020List = co2020Repository.findAll();
		int i = 1;
		for (Co2020 co2020 : co2020List) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(co2020.getCo2020Id());
			cell = row.createCell(1);
			cell.setCellValue(co2020.getNumero());
			cell = row.createCell(2);
			cell.setCellValue(co2020.getDate());
			cell = row.createCell(3);
			cell.setCellValue(co2020.getNomEntreprise());
			cell = row.createCell(4);
			cell.setCellValue(co2020.getAdresse());
			cell = row.createCell(5);
			cell.setCellValue(co2020.getDestination());
			cell = row.createCell(6);
			cell.setCellValue(co2020.getProduit());
			cell = row.createCell(7);
			cell.setCellValue(co2020.getQuantiteExporte());
			cell = row.createCell(8);
			cell.setCellValue(co2020.getUnite());
			cell = row.createCell(9);
			cell.setCellValue(co2020.getNombreConteneur());
			cell = row.createCell(10);
			cell.setCellValue(co2020.getPrixUnitaire());
			cell = row.createCell(11);
			cell.setCellValue(co2020.getMontant());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
        ...
        while (rowIterator.hasNext()) {
            ...
            Co2020 certi = new Co2020();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                certi.setCo2020Id(Integer.parseInt(cellValue));
                cell = cellIterator.next();
            }
            
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            certi.setNumero(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setDate(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setNomEntreprise(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setAdresse(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setDestination(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setProduit(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	certi.setQuantiteExporte(Integer.parseInt(cellValue));
            }

            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setUnite(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	certi.setNombreConteneur(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certi.setPrixUnitaire(cellValue);
