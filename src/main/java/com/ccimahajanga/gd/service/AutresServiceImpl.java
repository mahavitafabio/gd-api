...
@Service
public class AutresServiceImpl implements AutresService {
	
	@Autowired
	private AutresRepository autresRepository;
	
	@Override
	public Iterable<Autres> get(String q) {
		// TODO Auto-generated method stub
		if (q != null && !"".equals(q.trim())) {
			return autresRepository.findByTitreOuvragesContaining(q);
		}
		return autresRepository.findAll();
	}

	@Override
	public void save(Autres ouvrages) {
		// TODO Auto-generated method stub
		autresRepository.save(ouvrages);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			autresRepository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Autres> autresList = autresRepository.findAll();
		int i = 1;
		for (Autres autres : autresList) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(autres.getAutresId());
			cell = row.createCell(1);
			cell.setCellValue(autres.getCodeOuvrages());
			cell = row.createCell(2);
			cell.setCellValue(autres.getCommentaire());
			cell = row.createCell(3);
			cell.setCellValue(autres.getDomaines());
			cell = row.createCell(4);
			cell.setCellValue(autres.getEtage());
			cell = row.createCell(5);
			cell.setCellValue(autres.getNombreExemplaire());
			cell = row.createCell(6);
			cell.setCellValue(autres.getRanger());
			cell = row.createCell(7);
			cell.setCellValue(autres.getTitreOuvrages());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
		List<String> headers 
		= new ArrayList<>(Arrays.asList("AUTRES_ID",
        ...
        while (rowIterator.hasNext()) {
            ...
            Autres ouvr = new Autres();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                ouvr.setAutresId(Integer.parseInt(cellValue));
