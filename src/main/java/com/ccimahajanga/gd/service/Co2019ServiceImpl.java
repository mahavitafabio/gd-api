...
@Service
public class Co2019ServiceImpl implements Co2019Service {

	@Autowired
	private Co2019Repository co2019Repository;
	
	@Override
	public Iterable<Co2019> get(String q) {
		if (q != null && !"".equals(q.trim())) {
			return co2019Repository.findByProduitContaining(q);
		}
		return co2019Repository.findAll();
	}

	@Override
	public void save(Co2019 co2019) {
		// TODO Auto-generated method stub
		co2019Repository.save(co2019);
	}

	@Override
	public void delete(List<Integer> idList) {
		for(Integer id : idList) {
			co2019Repository.deleteById(id);
		}
	...
	public void export(OutputStream outputStream) throws IOException {
		...
		Iterable<Co2019> co2019List = co2019Repository.findAll();
		int i = 1;
		for (Co2019 co2019 : co2019List) {
			Row row = sheet.createRow(i++);
			cell = row.createCell(0);
			cell.setCellValue(co2019.getCo2019Id());
			cell = row.createCell(1);
			cell.setCellValue(co2019.getNumero());
			cell = row.createCell(2);
			cell.setCellValue(co2019.getDate());
			cell = row.createCell(3);
			cell.setCellValue(co2019.getNomEntreprise());
			cell = row.createCell(4);
			cell.setCellValue(co2019.getAdresse());
			cell = row.createCell(5);
			cell.setCellValue(co2019.getDestination());
			cell = row.createCell(6);
			cell.setCellValue(co2019.getProduit());
			cell = row.createCell(7);
			cell.setCellValue(co2019.getQuantiteExporte());
			cell = row.createCell(8);
			cell.setCellValue(co2019.getUnite());
			cell = row.createCell(9);
			cell.setCellValue(co2019.getNombreConteneur());
			cell = row.createCell(10);
			cell.setCellValue(co2019.getPrixUnitaire());
			cell = row.createCell(11);
			cell.setCellValue(co2019.getMontant());
		}
	...
	public void upload(MultipartFile multipart) throws IllegalStateException, IOException, InvalidFormatException {
        ...
        while (rowIterator.hasNext()) {
            ...
            Co2019 certif = new Co2019();
            Cell cell = cellIterator.next();
            String cellValue;
            if (!isInsert) {
                cellValue = dataFormatter.formatCellValue(cell);
                certif.setCo2019Id(Integer.parseInt(cellValue));
                cell = cellIterator.next();
            }
            
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            certif.setNumero(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setDate(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setNomEntreprise(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setAdresse(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setDestination(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setProduit(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	certif.setQuantiteExporte(Integer.parseInt(cellValue));
            }

            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setUnite(cellValue);
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            if (cellValue != null || !"".equals(cellValue)) {
            	certif.setNombreConteneur(Integer.parseInt(cellValue));
            }
            
            cell = cellIterator.next();
            cellValue = dataFormatter.formatCellValue(cell);
            certif.setPrixUnitaire(cellValue);
